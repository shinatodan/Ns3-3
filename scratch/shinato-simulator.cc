...
void
VanetRoutingExperiment::ParseCommandLineArguments (int argc, char **argv)//コマンドライン引数を処理する　argc:引数の数,argv:引数
{
    ...
    cmd.Parse (argc, argv);//プログラムの引数を解析する。argc:引数の数(最初の要素としてメインプログラムの名前を含む),argv:nullで終わる文字列の配列,それぞれがコマンドライン引数を識別する
    ConfigureDefaults();
}
...
VanetRoutingExperiment::ConfigureDevices ()//チャネルを構成する
{
    ...
    wifiChannel.SetPropagationDelay ("ns3::ConstantSpeedPropagationDelayModel");//このチャネルの電波伝搬遅延モデルを設定する(電波伝搬速度は一定の速度2.99792e+08 )
    ...
    wifiPhy.SetPcapDataLinkType (YansWifiPhyHelper::DLT_IEEE802_11);//pcapトレースのデータリンクタイプをieee802.11無線LANヘッダーで設定する
...
void
VanetRoutingExperiment::ConfigureMobility ()//モビリティを設定する
{
...
VanetRoutingExperiment::ConfigureApplications ()
{
    ...
    for (uint32_t i=0; i<totalsrcdst; i++){
        ...
        AddressValue remoteAddress (InetSocketAddress (adhocInterfaces.GetAddress(dst), Port));
        onoff1.SetAttribute ("Remote", remoteAddress);

        //送信ノード
        ApplicationContainer temp = onoff1.Install (adhocNodes.Get (src));
        
        double startTime =double(rand()%11)/10.0+1.0;
        temp.Start (Seconds (startTime));
        temp.Stop (Seconds (totalSimTime));
    }
}
